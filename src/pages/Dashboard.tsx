...
export default function Dashboard() {
  ...
  const getItemsSummary = (items: { productName: string; weight: number }[]) => {
    if (items.length === 1) return `${items[0].productName} - ${items[0].weight} kg`;
                ...
                {recentPurchases.map((purchase) => (
                  <div key={purchase.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      {purchase.balanceAmount > 0 && <Badge variant="destructive" className="text-xs">Due: ₹{purchase.balanceAmount}</Badge>}
                    </div>
                  </div>
                ))}
              </div>
                ...
                {recentSales.map((sale) => (
                  <div key={sale.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      {sale.balanceAmount > 0 && <Badge variant="destructive" className="text-xs">Due: ₹{sale.balanceAmount}</Badge>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {lowStockProducts.map((product) => (
                <div key={product.id} className="flex items-center justify-between p-3 bg-warning/10 rounded-lg border border-warning/20">
                  ...
                  <Badge variant="outline" className="border-warning text-warning">{product.stock} {product.unit}</Badge>
                </div>
              ))}
            </div>
          </CardContent>
